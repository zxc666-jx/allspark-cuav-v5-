...
namespace MissionPlanner.Controls
{
    ...
    { "calibration",        Calibration::create },
        ...
        private void pictureBoxplane_Click(object sender, EventArgs e)
        {
            ...
            string exepath = Settings.GetUserDataDirectory() + "sitl" + Path.DirectorySeparatorChar + "ArduPlane.exe";
            //如果文件不存在，则从远端下载
        ...
        private void pictureBoxrover_Click(object sender, EventArgs e)
        {
            ...
            string exepath = Settings.GetUserDataDirectory() + "sitl" + Path.DirectorySeparatorChar + "APMrover2.exe";
            //如果文件不存在，则从远端下载
            if (!File.Exists(exepath))
            {
                exepath = CheckandGetSITLImage("APMrover2.elf");
            }
        ...
        private void pictureBoxquad_Click(object sender, EventArgs e)
        {
            ...
            string exepath = Settings.GetUserDataDirectory() + "sitl" + Path.DirectorySeparatorChar + "ArduCopter.exe";
            //如果文件不存在，则从远端下载
            if (!File.Exists(exepath))
            {
        ...
        private void pictureBoxheli_Click(object sender, EventArgs e)
        {
            ...
            string exepath = Settings.GetUserDataDirectory() + "sitl" + Path.DirectorySeparatorChar + "ArduHeli.exe";
            //如果文件不存在，则从远端下载
            if (!File.Exists(exepath))
            {
                exepath = CheckandGetSITLImage("ArduHeli.elf");
            }
        ...
        {
            if (File.Exists(sitldirectory + "sim_vehicle.py") || Download.getFilefromNet(
                ...
                sitldirectory + "sim_vehicle.py"))
            {
                ...
                foreach (Match match in matches)
                {
                    ...
                    {
                        if (File.Exists(sitldirectory + match.Groups[2].Value.ToString()) || Download.getFilefromNet(
            ...
            if (File.Exists(sitldirectory + "vehicleinfo.py") || Download.getFilefromNet(
                ...
                sitldirectory + "vehicleinfo.py"))
            {
                    ...
                    new JsonTextReader(File.OpenText(sitldirectory + "vehicleinfo.py")))
                {
                    ...
                    foreach (var fwtype in obj)
                    {
                        ...
                        {
                            if (File.Exists(sitldirectory + configs.ToString()) || Download.getFilefromNet(
                                "https://raw.githubusercontent.com/ArduPilot/ardupilot/master/Tools/autotest/" +
                                configs.ToString(),
                        ...
                        {
                            if (File.Exists(sitldirectory + config1.ToString()) || Download.getFilefromNet(
                                "https://raw.githubusercontent.com/ArduPilot/ardupilot/master/Tools/autotest/" +
                                config1.ToString(),
        ...
        private void StartSITL(string exepath, string model, string homelocation, string extraargs = "", int speedup = 1)
        {
            ...
            try
            {
                uavcount = int.Parse(txt_uavcount.Text);
            }
            catch
            {
                uavcount = 1;
            ...
            if (uavcount <= 1)
            {
                config = GetDefaultConfig(model);
                if (!string.IsNullOrEmpty(config))
                    extraargs += @" --defaults """ + config + @"""";
                extraargs += " " + txt_cmdline.Text.Trim() + " --sysid " + uavcount.ToString() + " ";
                if (chk_wipe.Checked)
                    extraargs += " --wipe ";
                DoSITL(exepath, model, index, homelocation,extraargs,speedup);
                // 检查是否启用自动连接
                ...
                try
                {
                    MainV2.Comports.Add(ConnectToSITL(5760));
                }
                catch (Exception ex)
                {
                    log.Error("Failed to connect to SITL on port 5760", ex);
                    CustomMessageBox.Show(Strings.Failed_to_connect_to_SITL_instance, Strings.ERROR);
                    return;
                }
            }
            else  // 多机的仿真 让cmd其他命令无效，转为控制飞机的起始TCP PORT
            {
                if (txt_cmdline.Text!="")
                {
                    try
                    {
                        int baseport = int.Parse(txt_cmdline.Text);
                        baseindex = (baseport - 5760)/10;
                    }
                    catch
                    {
                        baseindex = 0;
                ...
                for (index = 0; index < uavcount; index++)
                {
                    extraargs = "";
                    config = GetDefaultConfig(model);
                    if (!string.IsNullOrEmpty(config))
                        extraargs += @" --defaults """ + config + @"""";
                    extraargs += " --instance " + (index + baseindex).ToString() + " --sysid " + (index + 1 + baseindex).ToString() + " ";
                    if (chk_wipe.Checked)
                        extraargs += " --wipe ";
                    DoSITL(exepath, model,index + baseindex, homelocation, extraargs, speedup);
                }
                // 检查是否启用自动连接
                if (!checkBox_AutoConnect.Checked)
                {
                ...
                // 等待 SITL 实例初始化完成（冷启动需要 5-10 秒）
                Thread.Sleep(8000);
                try
                {
                    for (int a = 0; a < uavcount; a++)
                    {
                        int port = 5760 + (10 * (a + baseindex));
                        MainV2.Comports.Add(ConnectToSITL(port));
                    }
                }
                catch (Exception ex)
                {
                    log.Error("Failed to connect to one or more SITL instances", ex);
                    CustomMessageBox.Show(Strings.Failed_to_connect_to_SITL_instance, Strings.ERROR);
                    return;
                }
            }
        ...
        private MAVLinkInterface ConnectToSITL(int port)
        {
            ...
            if (tcpClient == null)
                throw new Exception("SITL instance on port " + port + " not ready after 15 retries");
        ...
        private void DoSITL(string exepath, string model, int a,string homelocation, string extraargs, int speedup)
        {
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            if (keyData == (Keys.Control | Keys.S))
            {
                ...
                for (int a = max; a >= 0 ; a--)
                {
                    ...
                    {
                        extra += String.Format(
                            " -M{4} -s1 --home {3} --instance {0} --uartA tcp:0 {1} --sysid {2} ",
