...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        public FlightData()
        {
            ...
            ConfigureReadableFlightDataLayout();
            ...
            gMapControl1.Overlays.Add(virtualtargetoverlay);
            ...
            for (int f = 1; f < 30; f++)
            {
                ...
                if (Settings.Instance["quickView" + f] != null)
                {
                    ...
                    if (ctls.Length > 0)
                    {
                        ...
                        string desc = GetValidQuickViewField(QV, Settings.Instance["quickView" + f]);
                        QV.Tag = desc;
        ...
        private void mainloop()
        {
            ...
            DateTime aircraftMarkersUpdate = DateTime.MinValue;
            ...
            while (threadrun)
            {
                ...
                try
                {
                    ...
                    // Keep aircraft movement smooth without forcing the much heavier
                    // route, waypoint, ADS-B and camera overlays to redraw at 10 Hz.
                    if (aircraftMarkersUpdate.AddMilliseconds(AircraftMarkerRefreshIntervalMs) <= DateTime.Now)
                    {
                        updateAircraftMarkers();
                        aircraftMarkersUpdate = DateTime.Now;
                    }
        ...
        private const int AircraftMarkerRefreshIntervalMs = 100;

        private void updateAircraftMarkers()
        {
            if (!IsHandleCreated || IsDisposed || aircraftOverlay == null)
                return;
        ...
        void cam_camimage(Image camimage)
        {
            ...
            if (UserVideoShow.instance != null)
            {
                UserVideoShow.camimage = camimage;
            }
        }
...
// square
        ...
        private void AddRestoreAllActionTabsMenu()
        {
            contextMenuStripactionstab.Items.Add(new ToolStripSeparator());
            var restore = new ToolStripMenuItem("显示全部标签");
            restore.Click += delegate
            {
                tabControlactions.TabPages.Clear();
                foreach (var tabPage in TabListOriginal)
                    tabControlactions.TabPages.Add(tabPage);
                saveTabControlActions();
                ThemeManager.ApplyThemeTo(tabControlactions);
            };
            contextMenuStripactionstab.Items.Add(restore);
        }
        ...
        public void loadTabControlActions()
        {
            ...
            EnsureEssentialActionTabs();
            ThemeManager.ApplyThemeTo(tabControlactions);
        }

        private void ConfigureReadableFlightDataLayout()
        {
            // Multiple tab rows consume most of the action pane on scaled
            // displays. Keep one scrollable row and reserve room for controls.
            tabControlactions.Multiline = false;
            tabControlactions.SizeMode = TabSizeMode.Normal;
            SubMainLeft.Panel2MinSize = 150;
        }

        private void EnsureEssentialActionTabs()
        {
            TabPage[] essentialTabs = { tabQuick, tabActions, tabActionsSimple };
            int insertIndex = 0;
            foreach (TabPage tab in essentialTabs)
            {
                if (!tabControlactions.TabPages.Contains(tab))
                    tabControlactions.TabPages.Insert(Math.Min(insertIndex, tabControlactions.TabPages.Count), tab);
                insertIndex = tabControlactions.TabPages.IndexOf(tab) + 1;
            }
        }

        private static readonly string[] DefaultQuickViewFields =
        {
            "alt", "groundspeed", "wp_dist", "yaw", "verticalspeed", "DistToHome"
        };

        private void LoadSafeQuickViewLayout()
        {
            int cols = 2;
            int rows = 3;
            int savedCols;
            ...
            if (Int32.TryParse(Settings.Instance["quickViewCols"], out savedCols) &&
                Int32.TryParse(Settings.Instance["quickViewRows"], out savedRows) &&
                savedCols > 0 && savedRows > 0 && savedCols <= 3 && savedRows <= 4 &&
                savedCols * savedRows <= 12)
            {
                cols = savedCols;
                rows = savedRows;
            }

            setQuickViewRowsCols(cols.ToString(), rows.ToString());
        }

        private string GetValidQuickViewField(QuickView quickView, string requestedField)
        {
            PropertyInfo property = typeof(CurrentState).GetProperty(requestedField ?? String.Empty,
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.IgnoreCase);
            if (property != null && property.CanRead)
                return property.Name;

            int slot;
            string fallback = DefaultQuickViewFields[0];
            if (Int32.TryParse(quickView.Name.Replace("quickView", String.Empty), out slot) &&
                slot >= 1 && slot <= DefaultQuickViewFields.Length)
            {
                fallback = DefaultQuickViewFields[slot - 1];
            }

            Settings.Instance[quickView.Name] = fallback;
            log.WarnFormat("Ignoring invalid QuickView field '{0}' for {1}; using '{2}'",
                requestedField, quickView.Name, fallback);
            return fallback;
        }
