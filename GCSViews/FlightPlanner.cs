...
namespace MissionPlanner.GCSViews
{
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
        ...
        public FlightPlanner()
        {
            ...
            // 搜索区域
            searchareaoverlay = new GMapOverlay("searcharea");
            MainMap.Overlays.Add(searchareaoverlay);

            virtualtargetoverlay = new GMapOverlay("virtual target");
            MainMap.Overlays.Add(virtualtargetoverlay);
        ...
        public static GMapOverlay searchareaoverlay; // 搜索区域图层
        ...
        GMapOverlay virtualtargetoverlay;
        ...
        GMapMarkerVirtualTarget CurrentVirtualTarget;
        bool isMouseDown;
        bool isMouseDraging;
        ...
        void MainMap_OnMarkerLeave(GMapMarker item)
        {
            if (!isMouseDown)
            {
                ...
                if (item is GMapMarkerVirtualTarget)
                {
                    CurrentVirtualTarget = null;
                }
            }
        }
        ...
        void MainMap_OnMarkerEnter(GMapMarker item)
        {
            if (!isMouseDown)
            {
                ...
                if (item is GMapMarkerVirtualTarget)
                {
                    CurrentVirtualTarget = item as GMapMarkerVirtualTarget;
                }
        ...
        */
            ...
            if (isMouseDown) // mouse down on some other object and dragged to here.
            {
                ...
                else
                {
                    ...
                    if (CurentRectMarker != null && CurentRectMarker.InnerMarker != null)
                    {
                        ...
                        else if (CurentRectMarker.InnerMarker.Tag.ToString().Contains("serachareapoint"))
                        {
                            try
                            {
                                drawingSerachAreaPolygon.Points[
                                    int.Parse(CurentRectMarker.InnerMarker.Tag.ToString().Replace("serachareapoint", "")) - 1] =
                                    new PointLatLng(MouseDownEnd.Lat, MouseDownEnd.Lng);
                                updateserachpolygonmarkerToolTipText();
                                MainMap.UpdatePolygonLocalPosition(drawingSerachAreaPolygon);
                                MainMap.Invalidate();
                            }
                            catch (Exception ex)
                            {
        ...
        void MainMap_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left && isMouseDown)
            {
                ...
                else if (CurrentVirtualTarget != null)
                {
                    PointLatLng pnew = MainMap.FromLocalToLatLng(e.X, e.Y);
                    CurrentVirtualTarget.Position = pnew;
                }
        ...
        public void AddSerachAreaPolygon()
        {
            //serachareapoints.Add(new PointLatLng(MouseDownStart.Lat, MouseDownStart.Lng));
            if (drawingSerachAreaPolygon == null)
            {
                List<PointLatLng> polygonPoints = new List<PointLatLng>();
                drawingSerachAreaPolygon = new GMapPolygon(polygonPoints, "serach area polygon");
                drawingSerachAreaPolygon.Fill = new SolidBrush(Color.FromArgb(50, Color.Blue));
                drawingSerachAreaPolygon.Stroke = new Pen(Color.Blue, 2);
                //drawingSerachAreaPolygon.IsHitTestVisible = false;
                searchareaoverlay.Polygons.Add(drawingSerachAreaPolygon);
            }
            if (searchareaoverlay.Polygons.Count == 0)
            {
                drawingSerachAreaPolygon.Points.Clear();
                searchareaoverlay.Polygons.Add(drawingSerachAreaPolygon);
            }
            // remove full loop is exists
            if (drawingSerachAreaPolygon.Points.Count > 1 &&
                drawingSerachAreaPolygon.Points[0] == drawingSerachAreaPolygon.Points[drawingSerachAreaPolygon.Points.Count - 1])
                drawingSerachAreaPolygon.Points.RemoveAt(drawingSerachAreaPolygon.Points.Count - 1);

            drawingSerachAreaPolygon.Points.Add(new PointLatLng(MouseDownStart.Lat, MouseDownStart.Lng));

            addserachpolygonmarker(drawingSerachAreaPolygon.Points.Count.ToString(), MouseDownStart.Lng, MouseDownStart.Lat, 0);
            MainMap.UpdatePolygonLocalPosition(drawingSerachAreaPolygon);
            MainMap.Invalidate();
        }
        private void addserachpolygonmarker(string tag, double lng, double lat, int alt)
        {
            try
            {
                PointLatLng point = new PointLatLng(lat, lng);
                ...
                m.ToolTipMode = MarkerTooltipMode.OnMouseOver;
                // 第一个点
                if (searchareaoverlay.Markers.Count < 1)
                {
                    m.ToolTipText = "0m";
                }
                else
                {
                    PointLatLngAlt nowpoint = new PointLatLngAlt(point.Lat, point.Lng);
                    PointLatLngAlt lastpoint = new PointLatLngAlt(searchareaoverlay.Markers[searchareaoverlay.Markers.Count - 2].Position.Lat, searchareaoverlay.Markers[searchareaoverlay.Markers.Count - 2].Position.Lng);
                    double distancenowtolast = nowpoint.GetDistance(lastpoint);
                    m.ToolTipText = distancenowtolast.ToString("f0")+"m";
                }
                //m.ToolTipText = "serachareapoint" + tag;
                m.Tag = "serachareapoint" + tag;
                ...
                searchareaoverlay.Markers.Add(m);
                searchareaoverlay.Markers.Add(mBorders);
            }
            catch (Exception ex)
            {
                log.Info(ex.ToString());
        ...
        void updateserachpolygonmarkerToolTipText()
        {
            for (int i = 0; i < searchareaoverlay.Markers.Count; i += 2)
            {
                if (i == 0)
                {
                    searchareaoverlay.Markers[i].ToolTipText = "0m";
                }
                else
                {
                    PointLatLngAlt nowpoint = new PointLatLngAlt(searchareaoverlay.Markers[i].Position.Lat, searchareaoverlay.Markers[i].Position.Lng);
                    PointLatLngAlt lastpoint = new PointLatLngAlt(searchareaoverlay.Markers[i - 2].Position.Lat, searchareaoverlay.Markers[i - 2].Position.Lng);
                    double distancenowtolast = nowpoint.GetDistance(lastpoint);
                    searchareaoverlay.Markers[i].ToolTipText = distancenowtolast.ToString("f0") + "m";
                }
            }
        }
...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        internal bool drawseracharea = false;
        private GMapPolygon drawingSerachAreaPolygon = null;  // 正在画的搜索区域
        private void serachAreaDesignerToolStripMeultem_Click(object sender, EventArgs e)
        {
            if (!drawseracharea)
            {
                drawseracharea = true;
        ...
        private void endSerachAreaToolStripMenultem_Click(object sender, EventArgs e)
        {
            if (drawseracharea)
            {
                drawseracharea = false;
                if (drawingSerachAreaPolygon != null)
                {
                    //drawingSerachAreaPolygon.IsHitTestVisible = true;
                    searchareaoverlay.Polygons.Add(drawingSerachAreaPolygon);
                    if (FlightData.instance != null)
                    {
                        ...
                        foreach (var point in drawingSerachAreaPolygon.Points)
                        {
                            searchPolygon.Points.Add(new PointLatLng(point.Lat,point.Lng));
                        }
                        FlightData.instance.searchareaoverlay.Polygons.Add(searchPolygon);
                    }
                    drawingSerachAreaPolygon = null;
                }
                foreach (var m in searchareaoverlay.Markers)
                {
                    m.IsVisible = false;
                }
            }
        ...
        private void clearSerachArea_Click(object sender, EventArgs e)
        {
            if (drawingSerachAreaPolygon != null)
            {
                drawingSerachAreaPolygon.Points.Clear();
            ...
            searchareaoverlay.Markers.Clear();
            if (FlightData.instance != null)
            {
                FlightData.instance.searchareaoverlay.Polygons.Clear();
            }
        }

        private void addVirtualTargetToolStripMenuItem_Click(object sender, EventArgs e)
        {
            PointLatLng virtaulpoint = new PointLatLng(MouseDownStart.Lat, MouseDownStart.Lng);
            virtualtargetoverlay.Markers.Add(
                new GMapMarkerVirtualTarget(virtaulpoint, 0)
                {
                    ToolTipMode = MarkerTooltipMode.OnMouseOver,
                    ToolTipText = "Virtual Target",
                    Tag = virtualtargetoverlay.Markers.Count,
                }
                );
        }

        private void deleteVirtualTargetToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (CurrentVirtualTarget != null)
            {
                virtualtargetoverlay.Markers.Remove(CurrentVirtualTarget);
                MainMap.Invalidate(true);
                CurrentVirtualTarget = null;
            }
        }

        private void clearVirtualTargetToolStripMenuItem_Click(object sender, EventArgs e)
        {
            virtualtargetoverlay.Markers.Clear();
            if (FlightData.instance != null)
            {
                FlightData.instance.virtualtargetoverlay.Markers.Clear();
            }
        }

        private void uploadVirtualTargetToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (FlightData.instance != null)
            {
                FlightData.instance.virtualtargetoverlay.Markers.Clear();
                foreach (GMapMarkerVirtualTarget marker in virtualtargetoverlay.Markers)
                {
                    GMapMarkerVirtualTarget gMapMarker = new GMapMarkerVirtualTarget(marker.Position,marker.heading)
                    {
                        ToolTipMode = MarkerTooltipMode.OnMouseOver,
                        ToolTipText = "Virtual Target",
                        Tag = virtualtargetoverlay.Markers.IndexOf(marker),
                    };
                    FlightData.instance.virtualtargetoverlay.Markers.Add(gMapMarker);
