...
namespace MissionPlanner.Swarm
{
    public partial class Grid : UserControl
    {
        ...
        public List<icon> showicons = new List<icon>();
        icon mouseover = null;
        bool ismousedown = false;
        public bool DrawTransLine = false;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (DrawTransLine)
            {
                foreach (icon ico in showicons)
                {
                    foreach (icon subico in icons)
                    {
                        if (ico.interf == subico.interf)
                        {
                            //e.Graphics.DrawLine(Pens.White, (ico.x-xdist/-2)* xline, -(ico.y+ ydist/-2)*yline, (subico.x - xdist / -2) * xline, -(subico.y + ydist / -2) * yline);
                            if (Vertical)
                            {
                                e.Graphics.DrawLine(Pens.White, (ico.x - xdist / -2) * xline, -(ico.z + ydist / -2) * yline, (subico.x - xdist / -2) * xline, -(subico.z + ydist / -2) * yline);
                            }
                            else
                            {
                                e.Graphics.DrawLine(Pens.White, (ico.x - xdist / -2) * xline, -(ico.y + ydist / -2) * yline, (subico.x - xdist / -2) * xline, -(subico.y + ydist / -2) * yline);
        ...
        public void UpdateIcon(MAVState mav, float x, float y, float z, bool movable,Color showcolor)
        {
