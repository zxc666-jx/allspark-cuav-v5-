...
namespace MissionPlanner
{
    public class MAVLinkInterface : MAVLink, IDisposable, IMAVLinkInterface, IMAVLinkInterfaceLogRead
    {
        ...
        public UdpSerial UdpMirrorStream { get; set; }
        public bool UdpMirrorStreamWrite { get; set; }
...
4. Try a diffrent usb port\n\n" +
        ...
        public void sendSwarmNavPosition(byte sysid, byte compid, float navalt)
        {
            mavlink_swarm_nav_position_t swarmnavposition = new mavlink_swarm_nav_position_t()
            {
                nav_alt = navalt
            };
            generatePacket((byte)MAVLINK_MSG_ID.SWARM_NAV_POSITION, swarmnavposition, sysid, compid);
        }
...
// flush on heartbeat - 1 seconds
                    ...
                    try
                    {
                        ...
                        if (UdpMirrorStream != null && UdpMirrorStream.IsOpen)
                        {
                            UdpMirrorStream.Write(buffer, 0, buffer.Length);

                            while (UdpMirrorStream.BytesToRead > 0)
                            {
                                byte[] buf = new byte[1024];

                                int len = UdpMirrorStream.Read(buf, 0, buf.Length);

                                if (UdpMirrorStreamWrite)
                                {
                                    BaseStream.Write(buf, 0, len);
                                }
                            }
