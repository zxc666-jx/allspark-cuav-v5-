...
namespace MissionPlanner
{
    public partial class temp : Form
    {
        ...
        private void AddJSBSimHilEntry()
        {
            // 保留现有 XPlane 按钮，并为 Python JSBSim 硬件在环监控程序
            // 追加一个独立入口。
            var row = tableLayoutPanel1.RowCount;
            tableLayoutPanel1.RowCount = row + 1;
            tableLayoutPanel1.RowStyles.Add(new RowStyle(SizeType.Absolute, 28F));
            tableLayoutPanel1.Height += 28;
            AutoScroll = true;
            ...
            {
                var form = new JSBSimHilControlForm();
                ThemeManager.ApplyThemeTo(form);
                form.Show(this);
            };
            var description = new Label
            {
                Text = "启动并控制 JSBSim 单机硬件在环环境",
                AutoSize = true,
                Anchor = AnchorStyles.Left,
                Margin = new Padding(6, 3, 3, 3)
            };
            tableLayoutPanel1.Controls.Add(button, 0, row);
            tableLayoutPanel1.Controls.Add(description, 1, row);
        }

        private void AddJSBSimFaultEntry()
        {
            var row = tableLayoutPanel1.RowCount;
            tableLayoutPanel1.RowCount = row + 1;
            tableLayoutPanel1.RowStyles.Add(new RowStyle(SizeType.Absolute, 28F));
            ...
            var button = new MyButton
            {
                ...
                Dock = DockStyle.Fill,
                Name = "BUT_jsbsim_faults",
                Margin = new Padding(4, 3, 4, 3)
            };
            button.Click += delegate
            {
                var form = new JSBSimFaultInjectionForm();
                ThemeManager.ApplyThemeTo(form);
                form.Show(this);
            };
            var description = new Label
            {
                Text = "配置动力、舵面、空速和气压高度故障",
                AutoSize = true,
                Anchor = AnchorStyles.Left,
                Margin = new Padding(6, 3, 3, 3)
            };
        ...
        private void AddJSBSimFleetEntry()
        {
            var row = tableLayoutPanel1.RowCount;
            tableLayoutPanel1.RowCount = row + 1;
            tableLayoutPanel1.RowStyles.Add(new RowStyle(SizeType.Absolute, 28F));
            ...
            var button = new MyButton
            {
                Text = "JSBSim HIL 多机",
                Dock = DockStyle.Fill,
                Name = "BUT_jsbsim_fleet",
                Margin = new Padding(4, 3, 4, 3)
            };
            button.Click += delegate
            {
                var form = new JSBSimFleetControlForm();
                ThemeManager.ApplyThemeTo(form);
                form.Show(this);
            };
            var description = new Label
            {
                Text = "独立管理多架 JSBSim HIL Worker",
                AutoSize = true,
                Anchor = AnchorStyles.Left,
                Margin = new Padding(6, 3, 3, 3)
            };
        ...
        /// <summary>
        /// 将旧开发工具页整理为三栏布局。设计器中的控件仍然复用，
        /// 因此所有既有按钮事件和功能保持不变。
        /// </summary>
        private void ConfigureDeveloperToolsLayout()
        {
            SuspendLayout();

            Text = "Mission Planner 开发工具";
            StartPosition = FormStartPosition.CenterScreen;
            AutoScroll = false;

            var workingArea = Screen.PrimaryScreen.WorkingArea;
            var width = Math.Min(1500, Math.Max(900, workingArea.Width - 60));
            var height = Math.Min(950, Math.Max(680, workingArea.Height - 60));
            Size = new Size(width, height);
            MinimumSize = new Size(Math.Min(1100, width), Math.Min(760, height));

            // 先保存设计器中直接放在窗体上的按钮，再将它们转移到中间栏。
            // 左侧表格内的按钮不会被此查询选中。
            var extraButtons = Controls.OfType<MyButton>().ToList();
            Controls.Clear();
            ...
            root.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 31F));
            root.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 25F));
            root.RowStyles.Add(new RowStyle(SizeType.Percent, 100F));
            Controls.Add(root);

            root.Controls.Add(BuildPrimaryToolsColumn(), 0, 0);
            root.Controls.Add(BuildExtraToolsColumn(extraButtons), 1, 0);
            root.Controls.Add(BuildSensorStatusColumn(), 2, 0);

            ResumeLayout(true);
        }

        /// <summary>创建左侧“常用与数据工具”栏。</summary>
        private Control BuildPrimaryToolsColumn()
        {
            ...
            tableLayoutPanel1.Width = scrollPanel.ClientSize.Width;
            tableLayoutPanel1.Height = tableLayoutPanel1.RowCount * 36 + 8;
            ...
            {
                control.Margin = control is MyButton
                    ? new Padding(5, 4, 7, 4)
                    : new Padding(7, 4, 5, 4);
                ...
                if (control is Label)
                {
                    ...
                    ((Label)control).AutoEllipsis = true;
                }
            }
        ...
        /// <summary>创建中间“其他开发工具”按钮栏。</summary>
        private static Control BuildExtraToolsColumn(IEnumerable<MyButton> buttons)
        {
            var group = new GroupBox
            {
                Text = "其他开发工具",
        ...
        /// <summary>创建右侧传感器状态栏。</summary>
        private Control BuildSensorStatusColumn()
        {
            var group = new GroupBox
            {
                Text = "传感器状态",
            ...
            group.Controls.Add(controlSensorsStatus1);
            return group;
        }
        ...
        private void BUT_swarm_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBox.Show("窗口已经打开");
                return;
            }
            string password = "";
            if (InputBox.Show("高级权限", "请输入密码", ref password, true) == DialogResult.OK)
            {
