...
namespace MissionPlanner
{
    public partial class MainV2 : Form
    {
        ...
        private WM_DEVICECHANGE_enum _pendingDeviceChange = WM_DEVICECHANGE_enum.DBT_DEVNODES_CHANGED;
        private bool _deviceChangePending;
        private DateTime _lastDeviceChangeDispatch = DateTime.MinValue;
        ...
        private void ConfigureReadableMainMenu()
        {
            foreach (ToolStripItem item in MainMenu.Items)
            {
                if (item is ToolStripButton)
                    item.Font = new Font("Microsoft YaHei UI", 9F, FontStyle.Bold, GraphicsUnit.Point);
            }
        }
        ...
        public MainV2()
        {
            ...
            InitializeComponent();
            ConfigureReadableMainMenu();
            InitializeDeviceChangeDebounce();
            try
            {
        ...
        private void MenuSimulation_Click(object sender, EventArgs e)
        {
            ...
            Simulation = MyView.current?.Control as Controls.SITL;
        }
        ...
        public void doConnect(MAVLinkInterface comPort, string portname, string baud, bool getparams = false) //不下载参数列表20230728王新华
        {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            MyView.AddScreen(new MainSwitcher.Screen("Simulation", typeof(Controls.SITL), true));
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            if (keyData == (Keys.Control | Keys.F)) // 开发工具
            {
                Form frm = new temp();
        ...
        protected override void WndProc(ref Message m)
        {
            switch (m.Msg)
            {
                ...
                case WM_DEVICECHANGE:
                    QueueDeviceChange((WM_DEVICECHANGE_enum) m.WParam);
                    break;
        ...
        private void InitializeDeviceChangeDebounce()
        {
            _deviceChangeDebounceTimer = new System.Windows.Forms.Timer(components)
            {
                Interval = 1500
            };
        ...
        private void QueueDeviceChange(WM_DEVICECHANGE_enum cause)
        {
            if (_deviceChangeDebounceTimer == null)
                return;

            // A flight controller can enumerate several USB interfaces and can
            // briefly reconnect while booting. Treat that burst as one change.
            if (!_deviceChangePending || cause == WM_DEVICECHANGE_enum.DBT_DEVICEARRIVAL ||
                _pendingDeviceChange != WM_DEVICECHANGE_enum.DBT_DEVICEARRIVAL)
            {
                _pendingDeviceChange = cause;
            }
        ...
        private void DeviceChangeDebounceTimer_Tick(object sender, EventArgs e)
        {
            ...
            if ((DateTime.UtcNow - _lastDeviceChangeDispatch).TotalSeconds < 10)
                return;

            _lastDeviceChangeDispatch = DateTime.UtcNow;
            WM_DEVICECHANGE_enum cause = _pendingDeviceChange;
            log.DebugFormat("USB device changes settled: {0}", cause);
            ...
            {
                log.Warn("Device change callback failed", ex);
            }

            MissionPlanner.Plugin.Plugin[] plugins;
            lock (MissionPlanner.Plugin.PluginLoader.Plugins)
                plugins = MissionPlanner.Plugin.PluginLoader.Plugins.ToArray();

            foreach (var item in plugins)
                item.Host.ProcessDeviceChanged(cause);
        }
        ...
        private void MenuArduPilot_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //System.Diagnostics.Process.Start("http://wmdw.jswmw.com/home/?lid=1175");
                System.Diagnostics.Process.Start("http://www.graduate.nuaa.edu.cn/");
            }
            catch
            {
                //CustomMessageBox.Show("Failed to open url http://ardupilot.org");
                CustomMessageBox.Show("网址打开失败");
            }
        }
        ...
        // 显示用户视频流
        private void videoShowToolStripMenuItem_Click(object sender, EventArgs e)
        {
            new UserVideoShow().Show();
        }
    }
}
